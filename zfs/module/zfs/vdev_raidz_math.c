 ...
 */
#if defined(__x86_64) && defined(XXXHAVE_AVX512F)	/* only x86_64 for now */
	&vdev_raidz_avx512f_impl,
#endif
#if defined(__x86_64) && defined(XXXHAVE_AVX512BW)	/* only x86_64 for now */
...
#define	RAIDZ_IMPL_READ(i)	(*(volatile uint64_t *) &(i))

static uint64_t zfs_vdev_raidz_impl = IMPL_SCALAR;
...
vdev_raidz_math_get_ops()
{
	...
	const uint64_t impl = RAIDZ_IMPL_READ(zfs_vdev_raidz_impl);
...
static void *
raidz_math_kstat_addr(kstat_t *ksp, long long n)
{
...
vdev_raidz_math_init(void)
{
	...
	atomic_swap_64(&zfs_vdev_raidz_impl, user_sel_impl);
	raidz_math_initialized = B_TRUE;
}
...
vdev_raidz_impl_set(const char *val)
{
	...
	uint64_t impl = RAIDZ_IMPL_READ(user_sel_impl);
	size_t i;
	...
	if (err == 0) {
		if (raidz_math_initialized)
			atomic_swap_64(&zfs_vdev_raidz_impl, impl);
		else
...
int
zfs_vdev_raidz_impl_set(const char *val)
{
	return (vdev_raidz_impl_set(val));
}

int
zfs_vdev_raidz_impl_get(char *buffer, int max)
{
	int i, cnt = 0;
	char *fmt;
	const uint64_t impl = RAIDZ_IMPL_READ(zfs_vdev_raidz_impl);
	...
	for (i = 0; i < ARRAY_SIZE(math_impl_opts) - 2; i++) {
		...
		cnt += snprintf(buffer + cnt, max, fmt, math_impl_opts[i].name);
	}

	/* list all supported implementations */
	for (i = 0; i < raidz_supp_impl_cnt; i++) {
		fmt = (i == impl) ? "[%s] " : "%s ";
		cnt += snprintf(buffer + cnt, max, fmt, raidz_supp_impl[i]->name);
	}
