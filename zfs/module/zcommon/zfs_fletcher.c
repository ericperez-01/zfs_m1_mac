...
static uint64_t fletcher_4_impl_chosen = IMPL_FASTEST;

#define	IMPL_READ(i)	(*(volatile uint64_t *) &(i))
...
{
	int err = -EINVAL;
	uint64_t impl = IMPL_READ(fletcher_4_impl_chosen);
	size_t i, val_len;
	...
	if (err == 0) {
		atomic_swap_64(&fletcher_4_impl_chosen, impl);
		membar_producer();
	}
...
fletcher_4_impl_get(void)
{
	...
	const uint64_t impl = IMPL_READ(fletcher_4_impl_chosen);
...
static void *
fletcher_4_kstat_addr(kstat_t *ksp, int64_t n)
{
...
fletcher_4_benchmark_impl(boolean_t native, char *data, uint64_t data_size)
{
	...
	zio_cksum_t zc;
	uint32_t i, l;
	uint64_t sel_save = IMPL_READ(fletcher_4_impl_chosen);
	uint32_t keep_fastest = 0;
...
#ifdef __APPLE__
		dprintf("%s: %14s %-8s %16llu B/s\n",
			__func__,
			fletcher_4_supp_impls[i]->name,
			native ? "native" : "byteswap",
			run_bw);
#endif
	}

	/* restore original selection */
	atomic_swap_64(&fletcher_4_impl_chosen, sel_save);


#ifdef __APPLE__
	dprintf("%s: %14s %-8s chosen.\n",
		__func__,
		fletcher_4_supp_impls[keep_fastest]->name,
		native ? "native" : "byteswap");
#endif
...
int
zfs_fletcher_4_impl_get(char *buffer, int max)
{
	const uint64_t impl = IMPL_READ(fletcher_4_impl_chosen);
	char *fmt;
	int i, cnt = 0;
	...
	cnt += snprintf(buffer + cnt, max, fmt, "fastest");

	/* list all supported implementations */
	for (i = 0; i < fletcher_4_supp_impls_cnt; i++) {
		fmt = (i == impl) ? "[%s] " : "%s ";
		cnt += snprintf(buffer + cnt, max, fmt,
		    fletcher_4_supp_impls[i]->name);
	}
...
int
zfs_fletcher_4_impl_set(const char *val)
{
